...
#ifndef NAZARA_GRAPHICS_DEFERREDFRAMEPIPELINE_HPP
...
namespace Nz
{
	...
	class NAZARA_GRAPHICS_API DeferredFramePipeline : public FramePipeline
	{
		public:
			DeferredFramePipeline();
			DeferredFramePipeline(const DeferredFramePipeline&) = delete;
			DeferredFramePipeline(DeferredFramePipeline&&) = delete;
			~DeferredFramePipeline();

			void InvalidateViewer(std::size_t viewerIndex) override;
			void InvalidateWorldInstance(std::size_t) override;
			...
			void UnregisterMaterialPass(MaterialPass* materialPass) override;
			...
			DeferredFramePipeline& operator=(const DeferredFramePipeline&) = delete;
			DeferredFramePipeline& operator=(DeferredFramePipeline&&) = delete;
		...
		private:
			...
			struct ViewerData
			{
				std::size_t gbufferAlbedoAttachment;
				std::size_t gbufferNormalAttachment;
				std::size_t depthBufferAttachment;
				std::size_t depthPrepassOutput;
				std::size_t lightingOutputAttachment;
				std::unique_ptr<DepthPipelinePass> depthPrepass;
				std::unique_ptr<ForwardPipelinePass> forwardPass;
				std::unique_ptr<GBufferPipelinePass> gbufferPass;
				std::unique_ptr<LightingPipelinePass> lightingPass;
...
#include <Nazara/Graphics/DeferredFramePipeline.inl>

#endif // NAZARA_GRAPHICS_DEFERREDFRAMEPIPELINE_HPP
