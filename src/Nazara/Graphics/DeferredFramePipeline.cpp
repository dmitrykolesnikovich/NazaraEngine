...
namespace Nz
{
	DeferredFramePipeline::DeferredFramePipeline() :
	...
	void DeferredFramePipeline::InvalidateViewer(std::size_t viewerIndex)
	{
		m_invalidatedViewerInstances.Set(viewerIndex);
	}

	void DeferredFramePipeline::InvalidateWorldInstance(std::size_t worldInstanceIndex)
	{
		m_invalidatedWorldInstances.Set(worldInstanceIndex);
	}

	std::size_t DeferredFramePipeline::RegisterLight(std::shared_ptr<Light> light, UInt32 renderMask)
	{
		...
		lightData->onLightInvalidated.Connect(lightData->light->OnLightDataInvalided, [=](Light*)
		{
			...
			for (auto& viewerData : m_viewerPool)
			{
				...
				if (viewerRenderMask & renderMask)
				{
					...
					viewerData.lightingPass->InvalidateLights();
				}
			}
		});
	...
	void DeferredFramePipeline::RegisterMaterialPass(MaterialPass* materialPass)
	{
	...
	std::size_t DeferredFramePipeline::RegisterRenderable(std::size_t worldInstanceIndex, const InstancedRenderable* instancedRenderable, UInt32 renderMask, const Recti& scissorBox)
	{
		...
		renderableData->onElementInvalidated.Connect(instancedRenderable->OnElementInvalidated, [=](InstancedRenderable* /*instancedRenderable*/)
		{
			...
			for (auto& viewerData : m_viewerPool)
			{
				...
				if (viewerRenderMask & renderMask)
				{
					...
					viewerData.gbufferPass->InvalidateElements();
					viewerData.forwardPass->InvalidateElements();
				}
			}
		});
		...
		renderableData->onMaterialInvalidated.Connect(instancedRenderable->OnMaterialInvalidated, [this](InstancedRenderable* instancedRenderable, std::size_t materialIndex, const std::shared_ptr<Material>& newMaterial)
		{
			if (newMaterial)
			{
				for (auto& viewerData : m_viewerPool)
				{
					...
					viewerData.gbufferPass->RegisterMaterial(*newMaterial);
			...
			if (prevMaterial)
			{
				for (auto& viewerData : m_viewerPool)
				{
					...
					viewerData.gbufferPass->UnregisterMaterial(*prevMaterial);
		...
		for (std::size_t i = 0; i < matCount; ++i)
		{
			if (Material* mat = instancedRenderable->GetMaterial(i).get())
			{
				for (auto& viewerData : m_viewerPool)
				{
					...
					viewerData.gbufferPass->RegisterMaterial(*mat);
	...
	std::size_t DeferredFramePipeline::RegisterViewer(AbstractViewer* viewerInstance, Int32 renderOrder)
	{
		...
		viewerData.gbufferPass = std::make_unique<GBufferPipelinePass>(*this, viewerInstance);
		viewerData.lightingPass = std::make_unique<LightingPipelinePass>(*this, viewerInstance);
	...
	std::size_t DeferredFramePipeline::RegisterWorldInstance(WorldInstancePtr worldInstance)
	{
	...
	void DeferredFramePipeline::Render(RenderFrame& renderFrame)
	{
		...
		for (auto& viewerData : m_viewerPool)
		{
			...
			std::size_t geometryVisibilityHash = 5U;
			...
			for (const RenderableData& renderableData : m_renderablePool)
			{
				...
				geometryVisibilityHash = CombineHash(geometryVisibilityHash, std::hash<const void*>()(&renderableData));
			}

			// Lights update don't trigger a rebuild of the depth pre-pass
			std::size_t depthVisibilityHash = geometryVisibilityHash;

			std::size_t lightVisibilityHash = 13U;
			...
			for (const LightData& lightData : m_lightPool)
			{
				...
				if (renderMask & lightData.renderMask && frustum.Contains(boundingVolume))
				{
					...
					lightVisibilityHash = CombineHash(lightVisibilityHash, std::hash<const void*>()(lightData.light.get()));
				}
			}

			std::size_t visibilityHash = CombineHash(depthVisibilityHash, lightVisibilityHash);
			...
			viewerData.gbufferPass->Prepare(renderFrame, frustum, m_visibleRenderables, depthVisibilityHash);
			viewerData.lightingPass->Prepare(renderFrame, frustum, m_visibleLights, lightVisibilityHash);
		}
				...
				viewerData.blitShaderBinding->Update({
					{
						...
						ShaderBinding::TextureBinding {
							m_bakedFrameGraph.GetAttachmentTexture(viewerData.lightingOutputAttachment).get(),
							sampler.get()
						}
					}
				});
	...
	void DeferredFramePipeline::UnregisterLight(std::size_t lightIndex)
	{
		m_lightPool.Free(lightIndex);
	}

	void DeferredFramePipeline::UnregisterMaterialPass(MaterialPass* materialPass)
	{
		auto it = m_activeMaterialPasses.find(materialPass);
	...
	void DeferredFramePipeline::UnregisterRenderable(std::size_t renderableIndex)
	{
	...
	void DeferredFramePipeline::UnregisterViewer(std::size_t viewerIndex)
	{
	...
	void DeferredFramePipeline::UnregisterWorldInstance(std::size_t worldInstance)
	{
		// Defer world instance release
		m_removedWorldInstances.UnboundedSet(worldInstance);
	}

	void DeferredFramePipeline::UpdateLightRenderMask(std::size_t lightIndex, UInt32 renderMask)
	{
		LightData* lightData = m_lightPool.RetrieveFromIndex(lightIndex);
		lightData->renderMask = renderMask;
	}

	void DeferredFramePipeline::UpdateRenderableRenderMask(std::size_t renderableIndex, UInt32 renderMask)
	{
		RenderableData* renderableData = m_renderablePool.RetrieveFromIndex(renderableIndex);
		renderableData->renderMask = renderMask;
	}

	void DeferredFramePipeline::UpdateRenderableScissorBox(std::size_t renderableIndex, const Recti& scissorBox)
	{
		RenderableData* renderableData = m_renderablePool.RetrieveFromIndex(renderableIndex);
		renderableData->scissorBox = scissorBox;
	...
	void DeferredFramePipeline::UpdateViewerRenderMask(std::size_t viewerIndex, Int32 renderOrder)
	{
		ViewerData* viewerData = m_viewerPool.RetrieveFromIndex(viewerIndex);
	...
	BakedFrameGraph DeferredFramePipeline::BuildFrameGraph()
	{
		FrameGraph frameGraph;

		for (auto& viewerData : m_viewerPool)
		{
			viewerData.gbufferAlbedoAttachment = frameGraph.AddAttachment({
				"GBuffer - Diffuse",
				PixelFormat::RGBA8
			});

			viewerData.gbufferNormalAttachment = frameGraph.AddAttachment({
				"GBuffer - Normal",
				PixelFormat::RGBA8
			});

			viewerData.depthBufferAttachment = frameGraph.AddAttachment({
				"Depth-stencil buffer",
				Graphics::Instance()->GetPreferredDepthFormat()
			});

			viewerData.depthPrepassOutput = frameGraph.AddAttachmentProxy("Depth prepass output", viewerData.depthBufferAttachment);

			viewerData.lightingOutputAttachment = frameGraph.AddAttachment({
				"Lighting output",
				PixelFormat::RGBA32F
			});

			if (viewerData.depthPrepass)
				viewerData.depthPrepass->RegisterToFrameGraph(frameGraph, viewerData.depthBufferAttachment);

			viewerData.gbufferPass->RegisterToFrameGraph(frameGraph, viewerData.gbufferAlbedoAttachment, viewerData.gbufferNormalAttachment, viewerData.depthPrepassOutput);
			viewerData.lightingPass->RegisterToFrameGraph(frameGraph, viewerData.gbufferAlbedoAttachment, viewerData.gbufferNormalAttachment, viewerData.depthPrepassOutput, viewerData.lightingOutputAttachment);
		...
		for (auto&& [renderTarget, renderTargetData] : m_renderTargets)
		{
			...
			for (const ViewerData* viewerData : targetViewers)
				mergePass.AddInput(viewerData->lightingOutputAttachment);
